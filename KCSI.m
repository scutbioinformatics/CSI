function [A,Z,err]=KCSI(X,c,lambda,alpha,beta,maxIter)
...
Z = cell(1,m);
for v = 1:m
%     Z{v}=inv(X{v}'*X{v}+0.1)*(X{v}'*X{v});
%     Z{v}=zeros(n);
    Z{v}=eye(n);
    K{v}=myKernel(X{v}',2018);
    pW =constructW(X{v}');
    pD = diag(sum(pW));
    pL{v} = pD-pW;
    W0 = W0 + pW/3;
...
options.Display = 'off';
% options.Algorihtm = 'active-set';
% options.MaxIterations = 50;
% options.OptimalityTolerance = 1e-1;
H = m*eye(n);
H = (H'+ H)/2;
d = zeros(n,n);
...
%         A(:,i) = qpas(H,f,[],[],ones(1,n),1,zeros(n,1),[]);
        A(:,i) = quadprog(H,f,[],[],ones(1,n),1,zeros(n,1),ones(n,1),[],options);
    end
    % update Zv
    for v = 1:m
        temp = (K{v}+lambda*eye(n)+alpha*pL{v})\(K{v}+lambda*A);
        Z{v} = temp - temp.*eye(n);
    end
    % update P
    ...
    for i = 1:m
        sum1 = sum1+trace(K{v}-2*K{v}*Z{v}+Z{v}'*K{v}*Z{v});
